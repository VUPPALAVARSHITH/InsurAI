...
public ResponseEntity<?> getFraudClaims(@RequestHeader(value = "Authorization") String authHeader) {
    ...
    // ================= HR WORKLOAD COUNTER =================
    @GetMapping("/workload")
    public ResponseEntity<?> getHrWorkload() {

        Map<Long, Long> workloadMap = hrService.getHrPendingClaimCounts();

        List<Map<String, Object>> response = hrRepository.findAll()
                .stream()
                .map(hr -> Map.<String, Object>of(
                        "hrId", hr.getId(),
                        "hrName", hr.getName(),
                        "pendingClaims", workloadMap.getOrDefault(hr.getId(), 0L)
                ))
                .collect(Collectors.toList());
